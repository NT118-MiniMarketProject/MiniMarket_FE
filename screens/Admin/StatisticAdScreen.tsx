...
const StatisticAdScreen: React.FC = () => {
  ...
  const getStatistics = async (quarter: string, year: string) => {
    ...
    try {
      const response = await axios.post(
        "https://minimarket-be.onrender.com/api/v1/sales/statistics",
        {
          quarter: quarter,
          year: year,
        },
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text className="text-2xl text-center font-bold mb-4 mt-8">
        Thống kê bán hàng
      </Text>
      ...
      {stats && (
        <View style={styles.statsContainer}>
          <Text className="font-bold text-lg">
            Tổng đơn hàng:{" "}
            <Text className="font-normal">{stats.totalOrders}</Text>
          </Text>
          <Text className="font-bold text-lg">
            Tổng doanh thu:{" "}
            <Text className="font-normal">{stats.totalRevenue}</Text>
          </Text>
          <Text className="font-bold text-lg">Sản phẩm phổ biến nhất:</Text>
          <Image
            style={styles.productImage}
            source={{ uri: stats.popularProduct.thumbnail }}
          />
          <Text className="text-red-500 font-bold text-2xl text-center">
            {stats.popularProduct.name}
          </Text>
          <Text className="text-red-500 font-bold text-2xl text-center">
            Được {stats.popularProductBoughtTimes} lần mua
          </Text>
        </View>
      )}
    </ScrollView>
