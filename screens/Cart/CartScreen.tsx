            ...
            <View className="w-full">
              ...
              (cartData?.data && cartData.data.cartItems.length) ? (
                cartData.data.cartItems.map((item, index) => (
                  <View className="flex-row relative items-center space-x-1 w-full mb-1" key={index}>
                    {/* Image */}
                    ...
                    {/* Delete items  */}
                    <TouchableOpacity className="absolute w-6 h-6 flex-row items-center justify-center left-0 top-0 rounded-full bg-gray-200" onPress={() =>{
